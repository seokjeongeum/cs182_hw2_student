...
def rnn_backward(dh, cache):
    ...
    for t in range(T - 1, -1, -1):
        dx[:, t, :], dh0, Wx, Wh, b = rnn_step_backward(dh[:, t, :] + dh0, cache[t])
        dWx += Wx
        dWh += Wh
        db += b
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    product = dnext_h * o * (1 - np.tanh(next_c) ** 2) + dnext_c
    di = g * product * i * (1 - i)
    df = prev_c * product * f * (1 - f)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    prev_c = np.zeros((N, H))
    for t in range(T):
        h0, prev_c, cache[t] = lstm_step_forward(x[:, t, :], h0, prev_c, Wx, Wh, b)
        h[:, t, :] = h0
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_backward(dh, cache):
    ...
    db = np.zeros((4 * H,))
    for t in range(T - 1, -1, -1):
        dx[:, t, :], dh0, dc, Wx, Wh, b = lstm_step_backward(
            dh[:, t, :] + dh0, dc, cache[t]
        )
        dWx += Wx
        dWh += Wh
